...
var SquadRemoveRegex = regexp.MustCompile("/player_remove(?:@MuskratBot)?(?:\\s(?:@(.*)|(\\d+)))?")

func (m *Module) SquadRemove(c telebot.Context) error {
	scope := c.Get("scope").(permissions.Scope)

	// Check if message is in correct format
	if !SquadRemoveRegex.MatchString(c.Text()) {
		return c.Send(m.l.Text(c, "squad_remove_wrong_format"))
	}

	// Parse target
	target := SquadRemoveRegex.FindStringSubmatch(c.Text())
	...
	if errors.Is(err, domain.ErrNotInSquad) {
		return c.Send(m.l.Text(c, "squad_remove_not_in_squad"))
	} else if errors.Is(err, domain.ErrLeaderCannotLeave) {
		return c.Send(m.l.Text(c, "squad_remove_leader_can_not_leave"))
	} else if err != nil {
		return err
	}

	return c.Send(m.l.Text(c, "squad_remove_success", pl))
}
